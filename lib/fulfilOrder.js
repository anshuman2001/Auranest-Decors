...
async function fulfil(merchantOrderId, state, phonepeTxnId) {
  ...
  // First confirmer creates the Shiprocket shipment and alerts the owner
  ...
  if (!result.alreadyConfirmed && result.order) {
    ...
    notifyNewOrder({ ...result.order, orderId: merchantOrderId }).catch(e => console.error('[fulfil] notify:', e.message));
  }
